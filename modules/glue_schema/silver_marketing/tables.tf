locals {
  tables = {
    ...
    # Google Search Console – Silver Enriquecimento (S2S: normalizada -> enriquecida)
    gsc_metrics_enriched = {
      description = "Métricas GSC enriquecidas (Silver Enriquecimento): leitura da gsc_metrics, dedup e regras de negócio; particionadas por report_date."
      location    = "s3://${var.bucket}/domain=${var.domain}/source=google_search_console/dataset=gsc_metrics_enriched/"
